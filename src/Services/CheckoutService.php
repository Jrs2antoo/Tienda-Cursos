    ...
    public function completarCompra(int $userId): void
    {
        ...
        try {
            ...
            if ($usuario && !empty($cursos)) {
                $pdfBytes = FacturaPdf::generar($usuario->fullName, $usuario->email, $cursos);
                $filename = 'factura-' . date('Ymd') . '-' . $userId . '.pdf';

                $email = new Email($usuario->email, $usuario->fullName);
                $email->enviarFactura($pdfBytes, $filename);
            }
        } catch (\Throwable $e) {
